if (!isDedicated) then {
	...
	LOG_FNCT_CHAINING = { 
		...
		if (!LOG_CFG_ALLOW_CHAINING) then  { 
			_return = false; 
			if ( (_this select 1) == 1 && isNull (_target getVariable "LOG_trailer") ) then { _return = true;};
			if ( (_this select 1) == 2 && (isNull (_target getVariable "LOG_moves_by")||(!alive (_target getVariable "LOG_moves_by"))) ) then {_return = true;};
		};
		_return 
	};
